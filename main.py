...
def register():
    ...
    if User.query.filter_by(email=user_form.email.data).first():
      flash("You've already signed up with that email. Please sign in.")
      return redirect(url_for('login'))
    if user_form.validate_on_submit():
      new_user = User(
        email = user_form.email.data,
        name = user_form.name.data,
        password = generate_password_hash(password=user_form.password.data, method='pbkdf2:sha256', salt_length=8)
        )
...
def show_post(post_id):
    ...
    # hashed_email =  hashlib.md5(current_email.strip().lower().encode()).hexdigest()
